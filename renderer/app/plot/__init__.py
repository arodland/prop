...
matplotlib.rcParams['contour.algorithm'] = 'threaded'
import numpy as np
import sys
...
matplotlib.cm.register_cmap('turbo', cmap=ListedColormap(turbo_colormap_data), override_builtin=True)

def saturate(c, sat):
    gray = 0.2989*c[0] + 0.5870*c[1] + 0.1140*c[2]

    for i in range(3):
        c[i] = c[i]*(1+sat) - gray*sat

    return c

def scale_shift(c, a, b):
    return [c[0]*a+b, c[1]*a+b, c[2]*a+b, c[3]]
...
class Plot:
    def __init__(self, metric_name, date, decorations=True, basemaps="new", alpha=0.35, centered=None):
...
#            self.proj = ccrs.AzimuthalEquidistant(centered[0], centered[1])
        ...
        if self.decorations:
            if centered is None:
                self.ax.grid(visible=True, linewidth=.5, color='black', alpha=0.25, linestyle='--')
...
#            self.ax.background_patch.set_visible(False)
        ...
        if basemaps == "old":
            ...
            self.ax.add_feature(cartopy.feature.NaturalEarthFeature('physical', 'land', '110m',
                edgecolor=np.array((0x55, 0x55, 0x55))/256.,
                facecolor=np.array((0xdd, 0xdd, 0xcc))/256.,
                zorder=-3
                )
            )
            self.ax.add_feature(cartopy.feature.NaturalEarthFeature('physical', 'lakes', '110m',
                edgecolor='none',
                facecolor=cartopy.feature.COLORS['water'],
                zorder=-2
                )
            )
            self.ax.add_feature(cartopy.feature.NaturalEarthFeature('cultural', 'admin_0_boundary_lines_land', '110m',
                edgecolor=np.array((0x77, 0x77, 0x77))/256.,
                facecolor='never',
                zorder=-1
                )
            )
            self.ax.add_feature(cartopy.feature.NaturalEarthFeature('cultural', 'admin_1_states_provinces_lakes', '110m',
                edgecolor=np.array((0x77, 0x77, 0x77))/256.,
                facecolor='none',
                zorder=-1
                )
            )
            self.ax.add_feature(Nightshade(self.date, alpha=0.065))
    ...
    def scale_common(self, cmap_name):
        ...
        if self.basemaps == "old":
            ...
            self.cmap_light = map_cmap(lambda c: clip(scale_shift(c, self.plotalpha, 1-self.plotalpha)), self.cmap)
            self.cmap_dark = map_cmap(lambda c: clip(saturate(scale_shift(c, 0.6, 0.0), 0.75)), self.cmap)
    ...
    def draw_contour(self, zi, lon_min=-180, lon_max=180, lon_steps=361, lat_min=-90, lat_max=90, lat_steps=181):
        ...
        CS2 = plt.contour(
                ...
                norm=self.norm,
                linewidths=.6,
                alpha=.95 if self.basemaps == "new" else .75,
                transform=self.proj
                )
    ...
    def draw_title(self, metric, extra=None):
        title = metric + ' ' + str(self.date.strftime('%Y-%m-%d %H:%M'))
