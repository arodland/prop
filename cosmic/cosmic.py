...
def get_iri(tm, lat, lon):
    ...
    iri = subprocess.Popen(
        ['/build/iri2020_driver', str(tm.year), str(tm.month), str(tm.day), str(tm.hour),
         str(tm.minute), str(tm.second), str(lat), str(lon), '0', '0', '0'],
...
for key in sorted(bins.keys()):
    ...
    with con.cursor() as cur:
        ...
        for row in cur:
            ...
            sd = [ {'id': x['station.id'], 'latlon': LatLon(
                x['station.latitude'], x['station.longitude']) } for x in sd ]
            ...
            for record in records:
                ...
                ins.execute("insert into cosmic_eval(time, latitude, longitude, dip_angle, modip, run_id, hours_ahead, fof2_true, fof2_iri, fof2_irimap, fof2_full, hmf2_true, hmf2_iri, hmf2_irimap, hmf2_full, nearest_station, station_distance, source, generation) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, nextval('cosmic_eval_generation'))",
                            (record['ts'], record['lat'], record['lon'], record['dip_angle'], record['modip'], row['run_id'], row['hours_ahead'], fof2_true, fof2_iri, fof2_iri_essn, fof2_full, hmf2_true, hmf2_iri, hmf2_iri_essn, hmf2_full, nearest['id'], nearest['dist'], record['source']))
